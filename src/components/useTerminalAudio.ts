  ...
  useEffect(() => {
    ...
    try {
      ...
      if (enabled && AC) {
        ...
        ["/audio/mouse-click.mp3", "/audio/terminal-button.mp3", "/audio/terminal-simple-button.mp3", "/audio/terminal-power-button.mp3", "/audio/terminal-turning-on.mp3", "/audio/tick.mp3", "/audio/paper-slide.mp3", "/audio/gears.mp3", "/audio/lock-button-1.mp3", "/audio/lock-button-4.mp3", "/audio/lock-online-1.mp3", "/audio/lock-fail-1.mp3", "/audio/new-message.mp3", "/audio/my-message.mp3"].forEach(
          (src) => {
            fetch(src)
              .then((r) => r.arrayBuffer())
              .then((a) => ac.decodeAudioData(a))
